...
const TABS = [
  { id: 'site', label: 'Site Identity', icon: '🏢' },
...
function SectionCard({ title, right, children }) {
  return (
    <div className="border border-paper-200 rounded-lg overflow-hidden mb-6">
      <div className="px-4 py-3 bg-paper-50 border-b border-paper-200 flex items-center justify-between">
        <h2 className="text-xs font-mono uppercase tracking-wide text-ink-600 font-bold">{title}</h2>
        {right}
      </div>
...
export default function Settings() {
  ...
  return (
    <div className="px-4 py-6 lg:px-8 lg:py-10 max-w-3xl mx-auto">
      ...
      <div className="flex flex-wrap gap-1 border-b border-paper-200 mb-6">
        {TABS.map(tab => (
          <button
            key={tab.id}
            type="button"
      ...
      <form onSubmit={handleSave}>
        ...
        {activeTab === 'language' && (
          <SectionCard title="🌐 Content Language">
            <div>
        ...
        {activeTab === 'ai' && (
          <>
            <SectionCard
              title="✦ AI Provider"
              right={<span className="text-xs text-ink-400">Active: <strong className="text-ink-700">{activeProvider?.label}</strong></span>}
            >
              <div>
            ...
            <SectionCard title="🤖 AI Daily Budget">
        ...
        {activeTab === 'wordpress' && (
          <>
            <SectionCard title="🔄 WordPress Integration" right={<StatusBadge configured={settings?.wp_configured} />}>
              <div>
            ...
            <SectionCard title="📊 Google Analytics Integration" right={<StatusBadge configured={settings?.ga4_configured} />}>
              <p className="text-xs text-ink-400">
        ...
        {activeTab === 'submissions' && (
          <SectionCard title="🔗 Guest Submission Link">
